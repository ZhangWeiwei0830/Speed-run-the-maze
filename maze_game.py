...
# 你图上的起点
BLUE_START = (70, 70)     # 左上蓝
RED_START = (70, 350)     # 左下红
...
def rect_hits_wall(rect, wall_mask):
    """玩家的矩形跟草垛有没有撞上"""
    left = max(0, rect.left)
    right = min(WIDTH - 1, rect.right - 1)
    top = max(0, rect.top)
    bottom = min(HEIGHT - 1, rect.bottom - 1)
...
def main():
    ...
    while running:
        ...
        if winner is None:
            ...
            if rect_hits_wall(blue_rect, wall_mask):
                # 撞墙回退
            ...
            if rect_hits_wall(red_rect, wall_mask):
